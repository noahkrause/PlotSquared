...
public class DefaultPlotAreaManager implements PlotAreaManager {
      ...
      case 8:
        ...
        for (PlotArea area : this.plotAreas) {
          if (hash == area.worldhash) {
            if (area.contains(location.getX(), location.getZ()) && (!this.plotAreaHasCollision
                || world.equals(area.worldname))) {
              return area;
            }
          }
        }
  ...
  public void addPlotArea(PlotArea plotArea) {
    HashSet<PlotArea> localAreas = new HashSet<>(
        Arrays.asList(getPlotAreas(plotArea.worldname, null)));
