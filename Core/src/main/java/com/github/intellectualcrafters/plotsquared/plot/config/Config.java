...
public class Config {
  ...
  /**
   * Get the value for a node<br> Probably throws some error if you try to get a non existent key
   */
    ...
    if (instance != null) {
      ...
      if (field != null) {
        try {
          ...
          PS.debug(
              "Invalid configuration value: " + key + ": " + value + " in " + root.getSimpleName());
          e.printStackTrace();
        }
      }
    }
    ...
    try {
      ...
      for (Field field : clazz.getFields()) {
        ...
        if (create != null) {
          ...
          writer.write(
              spacing + toNodeName(field.getName() + ": ") + toYamlString(field.get(instance),
                  spacing) + lineSeparator);
        }
      }
