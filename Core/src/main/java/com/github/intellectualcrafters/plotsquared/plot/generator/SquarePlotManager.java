...
public abstract class SquarePlotManager extends GridPlotManager {
  ...
  public PlotId getPlotId(PlotArea plotArea, int x, int y, int z) {
    try {
      ...
      boolean[] merged = new boolean[]{rz <= pathWidthLower, rx > end, rz > end,
          rx <= pathWidthLower};
  ...
  public Location getPlotBottomLocAbs(PlotArea plotArea, PlotId plotId) {
    ...
    int pz = plotId.y;
    int x =
        (dpw.ROAD_OFFSET_X + (px * (dpw.ROAD_WIDTH + dpw.PLOT_WIDTH))) - dpw.PLOT_WIDTH - (int) Math
            .floor(dpw.ROAD_WIDTH / 2);
    int z =
        (dpw.ROAD_OFFSET_Z + (pz * (dpw.ROAD_WIDTH + dpw.PLOT_WIDTH))) - dpw.PLOT_WIDTH - (int) Math
            .floor(dpw.ROAD_WIDTH / 2);
