...
public class Set extends SubCommand {
        ...
        for (String component : components) {
          if (component.equalsIgnoreCase(args[0])) {
            if (!Permissions.hasPermission(player, C.PERMISSION_SET_COMPONENT.f(component))) {
              MainUtil
                  .sendMessage(player, C.NO_PERMISSION, C.PERMISSION_SET_COMPONENT.f(component));
              return false;
            }
            PlotBlock[] blocks;
            try {
              ...
              for (int i = 0; i < blocks.length; i++) {
                ...
                if (block == null) {
                  ...
                  StringComparison<PlotBlock>.ComparisonResult match = WorldUtil.IMP.getClosestBlock
                      (name);
                  if (match != null) {
  ...
  public boolean noArgs(PlotPlayer player) {
    ...
    if (plot != null) {
      newValues
          .addAll(Arrays.asList(plot.getManager().getPlotComponents(plot.getArea(), plot.getId())));
    }
