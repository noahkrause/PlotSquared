...
import com.github.intellectualcrafters.plotsquared.plot.object.LazyBlock;
  ...
  public void doJoinTask(final PlotPlayer player) {
    ...
    if (Settings.Teleport.ON_LOGIN && plot != null) {
      ...
      MainUtil.sendMessage(player,
          C.TELEPORTED_TO_ROAD.f() + " (on-login) " + "(" + plot.getId().x + ";" + plot.getId().y
              + ")");
    }
  }
    ...
    switch (type) {
      ...
      case BREAK_BLOCK:
        ...
        return !(!notifyPerms || MainUtil
            .sendMessage(player, C.FLAG_TUTORIAL_USAGE, C.FLAG_USE.s() + '/' + C.FLAG_BREAK.s()));
      case BREAK_HANGING:
        if (plot == null) {
          return Permissions
              .hasPermission(player, C.PERMISSION_ADMIN_INTERACT_ROAD.s(), notifyPerms);
