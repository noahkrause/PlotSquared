  ...
  public boolean onCommand(PlotPlayer player, String[] args) {
    ...
    switch (sub) {
      ...
      case "create": {
        ...
        int currentClusters = Settings.Limit.GLOBAL ? player.getClusterCount()
        ...
        int allowed = Permissions
            .hasPermissionRange(player, C.PERMISSION_CLUSTER_SIZE, Settings.Limit.MAX_PLOTS);
      ...
      case "helpers": {
        ...
        if (args.length != 3) {
          MainUtil
              .sendMessage(player, C.COMMAND_SYNTAX, "/plot cluster helpers <add|remove> <player>");
          return false;
        }
        PlotArea area = player.getApplicablePlotArea();
        if (area == null) {
      ...
      case "sethome":
        ...
        DBFunc
            .setPosition(cluster, relative.getX() + "," + relative.getY() + "," + relative.getZ());
