...
import com.github.intellectualcrafters.plotsquared.listener.WEManager;
...
public class DebugExec extends SubCommand {
      ...
      if (PS.get() != null) {
        ...
        if (file.exists()) {
          ...
          String script = StringMan.join(Files
                  .readLines(new File(
                          new File(PS.get().IMP.getDirectory() + File.separator + Settings.Paths.SCRIPTS),
                          "start.js"),
  ...
  public boolean onCommand(final PlotPlayer player, String[] args) {
    List<String> allowed_params =
        Arrays.asList("calibrate-analysis", "remove-flag", "stop-expire", "start-expire", "seen",
            "list-scripts");
    if (args.length > 0) {
      ...
      switch (arg) {
        ...
        case "help":
          MainUtil.sendMessage(player,
              "Possible sub commands: /plot debugexec <" + StringMan.join(allowed_params, "|")
                  + ">");
          return false;
        case "addcmd":
          try {
            final String cmd = StringMan.join(Files
                    .readLines(MainUtil.getFile(
                        new File(PS.get().IMP.getDirectory() + File.separator + Settings.Paths.SCRIPTS),
                        args[1]),
                        StandardCharsets.UTF_8),
                System.getProperty("line.separator"));
        ...
        case "list-scripts":
          ...
          paginate(player, allFiles, 8, page, new RunnableVal3<Integer, File, PlotMessage>() {
            ...
            public void run(Integer i, File file, PlotMessage message) {
              ...
              message.text("[").color("$3").text(String.valueOf(i)).color("$1").text("]")
                  .color("$3").text(' ' + name).color("$1");
            }
        ...
        case "allcmd":
          ...
          script =
              "_1=PS.getBasePlots().iterator();while(_1.hasNext()){plot=_1.next();if(" + args[1]
