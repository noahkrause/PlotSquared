...
import com.github.intellectualcrafters.plotsquared.plot.commands.Template;
import com.github.intellectualcrafters.plotsquared.plot.config.Settings;
import com.github.intellectualcrafters.plotsquared.plot.object.FileBytes;
...
public class HybridPlotManager extends ClassicPlotManager {
  ...
  public void exportTemplate(PlotArea plotArea) throws IOException {
    HashSet<FileBytes> files = new HashSet<>(
        Collections.singletonList(new FileBytes(Settings.Paths.TEMPLATES + "/tmp-data.yml",
            Template.getBytes(plotArea))));
    String dir =
        "schematics" + File.separator + "GEN_ROAD_SCHEMATIC" + File.separator + plotArea.worldname
            + File.separator;
    String newDir =
    ...
    try {
      ...
      if (sideroad.exists()) {
        files.add(
            new FileBytes(newDir + "sideroad.schematic", Files.readAllBytes(sideroad.toPath())));
      }
  ...
  public boolean createRoadEast(PlotArea plotArea, Plot plot) {
    ...
    Location pos2 = new Location(plotArea.worldname, bot.getX(), Math.min(getWorldHeight(), 255),
        top.getZ() + 1);
  ...
  public boolean createRoadSouth(PlotArea plotArea, Plot plot) {
    ...
    Location pos2 = new Location(plotArea.worldname, top.getX() + 1,
  ...
  /**
   * <p>Clearing the plot needs to only consider removing the blocks - This implementation has used
   * the setCuboidAsync function, as it is fast, and uses NMS code - It also makes use of the fact
   * that deleting chunks is a lot faster than block updates This code is very messy, but you don't
   * need to do something quite as complex unless you happen to have 512x512 sized plots. </p>
   */
  @Override
