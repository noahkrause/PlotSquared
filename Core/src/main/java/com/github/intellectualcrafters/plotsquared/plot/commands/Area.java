  ...
  public boolean onCommand(final PlotPlayer player, String[] args) {
    ...
    switch (args[0].toLowerCase()) {
        ...
        switch (args.length) {
            ...
            switch (args[1].toLowerCase()) {
              ...
              case "pos2":  // Set position 2 and finish creation for type=2 (partial)
                ...
                if (hasConfirmation(player)) {
                  CmdConfirm
                      .addPending(player, getCommandString() + " create pos2 (Creates world)", run);
                } else {
                  run.run();
                }
                return true;
          ...
          default: // Start creation
            ...
            for (int i = 2; i < args.length; i++) {
              ...
              if (pair.length != 2) {
                C.COMMAND_SYNTAX.send(player,
                    getCommandString() + " create [world[:id]] [<modifier>=<value>]...");
                return false;
              }
            ...
            if (pa.TYPE != 2) {
              ...
              if (hasConfirmation(player)) {
                CmdConfirm
                    .addPending(player, getCommandString() + ' ' + StringMan.join(args, " "), run);
              } else {
                run.run();
              }
              return true;
            ...
            MainUtil.sendMessage(player,
                "$1Go to the first corner and use: $2 " + getCommandString() + " create pos1");
            break;
        }
        return true;
      case "i":
        ...
        paginate(player, areas, 8, page, new RunnableVal3<Integer, PlotArea, PlotMessage>() {
          ...
          public void run(Integer i, PlotArea area, PlotMessage message) {
            ...
            message.text("[").color("$3")
                ...
                .text("]").color("$3")
                .text(' ' + name).tooltip(tooltip).command(getCommandString() + " info " + area)
                .color("$1").text(" - ")
                .color("$2")
      ...
      case "regenerate": {
        ...
        if (area.TYPE != 2) {
          MainUtil
              .sendMessage(player, "$4Stop the server and delete: " + area.worldname + "/region");
          return false;
        }
      ...
      case "tp":
        ...
        if (area.TYPE != 2) {
          ...
          center.setY(
              1 + WorldUtil.IMP.getHighestBlock(area.worldname, center.getX(), center.getZ()));
        }
      ...
      case "remove":
        MainUtil
            .sendMessage(player, "$1World creation settings may be stored in multiple locations:"
