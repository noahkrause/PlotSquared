package com.github.intellectualcrafters.plotsquared.plot.util.area;
...
public class QuadMap<T> {
  ...
  public void add(T area) {
    ...
    if (region.minX >= this.x) {
      if (region.minZ >= this.z) {
        if (this.one == null) {
          this.one = newInstance(this.newsize, this.x + this.newsize, this.z + this.newsize,
              this.min);
        }
        this.one.add(area);
        recalculateSkip();
        ...
        if (this.two == null) {
          this.two = newInstance(this.newsize, this.x + this.newsize, this.z - this.newsize,
              this.min);
        }
        this.two.add(area);
        recalculateSkip();
      ...
      if (region.minZ >= this.z) {
        if (this.four == null) {
          this.four = newInstance(this.newsize, this.x - this.newsize, this.z + this.newsize,
              this.min);
        }
        this.four.add(area);
        recalculateSkip();
        ...
        if (this.three == null) {
          this.three = newInstance(this.newsize, this.x - this.newsize, this.z - this.newsize,
              this.min);
        }
        this.three.add(area);
        recalculateSkip();
  ...
  public boolean intersects(RegionWrapper other) {
    return (other.minX <= this.x + this.size) && (other.maxX >= this.x - this.size) && (other.minZ
        <= this.z + this.size) && (other.maxZ
        >= this.z - this.size);
  }
