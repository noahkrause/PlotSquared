...
import com.github.intellectualcrafters.plotsquared.plot.object.Expression;
...
public class Merge extends SubCommand {
  ...
  public boolean onCommand(final PlotPlayer player, String[] args) {
    ...
    final int maxSize = Permissions
        .hasPermissionRange(player, "plots.merge", Settings.Limit.MAX_PLOTS);
    if (size - 1 > maxSize) {
    ...
    for (final UUID owner : uuids) {
      ...
      if (hasConfirmation(player)) {
        CmdConfirm
            .addPending(accepter, C.MERGE_REQUEST_CONFIRM.s().replaceAll("%s", player.getName()),
                run);
      } else {
        run.run();
      }
