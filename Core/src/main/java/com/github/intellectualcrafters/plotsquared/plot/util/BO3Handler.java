...
import com.github.intellectualcrafters.plotsquared.plot.object.BO3;
...
public class BO3Handler {
  ...
  /**
   * Save a plot as a BO3 file. - Use null for the player object if no player is applicable
   */
  ...
  public static void write(OutputStream stream, Plot plot, BO3 bo3) throws IOException {
    ...
    for (int i = 0; i < lines.size(); i++) {
      ...
      String result = StringMan
          .replaceAll(line, "%owner%", MainUtil.getName(plot.owner), "%alias%", plot.toString(),
              "%blocks%", bo3.getBlocks(), "%branches%",
              bo3.getChildren(),
