  ...
  public boolean onCommand(PlotPlayer player, String[] args) {
    ...
    switch (arg) {
      ...
      case "expired":
        ...
        plots = ExpireManager.IMP == null ? new ArrayList<Plot>()
  ...
  public void displayPlots(final PlotPlayer player, List<Plot> plots, int pageSize, int page,
    ...
    this.paginate(player, plots, pageSize, page, new RunnableVal3<Integer, Plot, PlotMessage>() {
      ...
      public void run(Integer i, Plot plot, PlotMessage message) {
        ...
        PlotMessage trusted =
            new PlotMessage().text(C.color(C.PLOT_INFO_TRUSTED.s()
                .replaceAll("%trusted%", MainUtil.getPlayerList(plot.getTrusted()))))
                .color("$1");
        PlotMessage members =
            new PlotMessage().text(C.color(C.PLOT_INFO_MEMBERS.s()
        ...
        PlotMessage flags = new PlotMessage()
            .text(C.color(C.PLOT_INFO_FLAGS.s().replaceAll("%flags%", strFlags))).color("$1");
        message.text("[").color("$3").text(i + "")
            ...
            .command("/plot info " + plot.getArea() + ";" + plot.getId()).color(color).text(" - ")
            .color("$2");
        String prefix = "";
