package com.github.intellectualcrafters.plotsquared.plot.util.helpmenu;
...
public class HelpObject {
  ...
  public HelpObject(final Command command, final String label) {
    _rendered = StringMan
        .replaceAll(C.HELP_ITEM.s(), "%usage%", command.getUsage().replaceAll("\\{label\\}", label),
            "[%alias%]",
  ...
  private String buildArgumentList(final Argument[] arguments) {
    ...
    for (final Argument<?> argument : arguments) {
      builder.append("[").append(argument.getName()).append(" (").append(argument.getExample())
          .append(")],");
    }
