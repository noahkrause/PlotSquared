...
public class Inbox extends SubCommand {
  ...
  public void displayComments(PlotPlayer player, List<PlotComment> oldComments, int page) {
    ...
    string.append(StringMan
        .replaceAll(C.COMMENT_LIST_HEADER_PAGED.s(), "%amount%", comments.length, "%cur", page + 1,
            "%max", totalPages + 1, "%word", "all")
        + '\n');
    ...
    for (int x = page * 12; x < max; x++) {
      ...
      string.append("&8[&7#").append(x + 1).append("&8][&7").append(comment.world).append(';')
          .append(comment.id).append("&8][&6")
          .append(comment.senderName).append("&8]").append(color).append(comment.comment)
          .append('\n');
    }
    ...
    if (args.length > 1) {
      switch (args[1].toLowerCase()) {
        ...
        case "clear":
          ...
          Optional<ArrayList<PlotComment>> comments = plot.getSettings()
              .getComments(inbox.toString());
