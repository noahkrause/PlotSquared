...
import com.github.intellectualcrafters.plotsquared.plot.object.stream.AbstractDelegateOutputStream;
...
public class MainUtil {
  ...
  /**
   * If the NMS code for sending chunk updates is functional<br> - E.g. If using an older version of
   * Bukkit, or before the plugin is updated to 1.5<br> - Slower fallback code will be used if
   * not.<br>
   */
  public static boolean canSendChunk = false;
  /**
   * Cache of mapping x,y,z coordinates to the chunk array<br> - Used for efficent world
   * generation<br>
   */
  ...
  public static PlotId getPlotId(Location location) {
    ...
    return area == null ? null
        : area.getPlotManager().getPlotId(area, location.getX(), location.getY(), location.getZ());
  }

  /**
   * This cache is used for world generation and just saves a bit of calculation time when checking
   * if something is in the plot area.
   */
      ...
      public void run() {
        try {
          ...
          try (OutputStream output = con.getOutputStream();
            ...
            writer
                .append("Content-Type: text/plain; charset=" + StandardCharsets.UTF_8.displayName())
                .append(CRLF);
            String param = "value";
  ...
  public static boolean resetBiome(PlotArea area, Location pos1, Location pos2) {
    ...
    if (!StringMan.isEqual(WorldUtil.IMP
            .getBiome(area.worldname, (pos1.getX() + pos2.getX()) / 2, (pos1.getZ() + pos2.getZ()) / 2),
        biome)) {
  ...
  /**
   * Hashcode of a boolean array.<br> - Used for traversing mega plots quickly.
   *
   * @return hashcode
   */
  ...
  /**
   * If rating categories are enabled, get the average rating by category.<br> - The index
   * corresponds to the index of the category in the config
   */
  ...
  public static void format(String info, final Plot plot, PlotPlayer player, final boolean full,
    ...
    String description = !descriptionFlag.isPresent() ? C.NONE.s()
        : Flags.DESCRIPTION.valueToString(descriptionFlag.get());

    StringBuilder flags = new StringBuilder();
    HashMap<Flag<?>, Object> flagMap = FlagManager
        .getPlotFlags(plot.getArea(), plot.getSettings(), true);
    if (flagMap.isEmpty()) {
      ...
      TaskManager.runTaskAsync(new Runnable() {
        ...
        public void run() {
          ...
          if (full && Settings.Ratings.CATEGORIES != null
            ...
            info = newInfo
                .replaceAll("%rating%", String.format("%.1f", plot.getAverageRating()) + '/' + max);
          }
          whenDone.run(info);
        }
      });
  ...
  /**
   * Get a list of names given a list of uuids.<br> - Uses the format {@link C#PLOT_USER_LIST} for
   * the returned string
   */
