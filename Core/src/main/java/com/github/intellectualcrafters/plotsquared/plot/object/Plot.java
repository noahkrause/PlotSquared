...
import com.github.intellectualcrafters.plotsquared.jnbt.CompoundTag;
import com.github.intellectualcrafters.plotsquared.listener.PlotListener;
...
import com.github.intellectualcrafters.plotsquared.plot.generator.SquarePlotWorld;
import com.github.intellectualcrafters.plotsquared.plot.util.BO3Handler;
...
public class Plot {
  ...
  /**
   * Session only plot metadata (session is until the server stops)<br> <br> For persistent metadata
   * use the flag system
   *
   * @see FlagManager
   */
  ...
  /**
   * Get the metadata for a key<br> <br> For persistent metadata use the flag system
   */
  ...
  public boolean isMerged() {
    return getSettings().getMerged(0) || getSettings().getMerged(2) || getSettings().getMerged(1)
        || getSettings().getMerged(3);
  }
  ...
  public Location getHome() {
    ...
    if (home == null || home.x == 0 && home.z == 0) {
      ...
      if (WorldUtil.IMP.getBlock(loc).id != 0) {
        loc.setY(
            Math.max(1 + WorldUtil.IMP.getHighestBlock(this.getWorldName(), loc.getX(), loc.getZ()),
                bot.getY()));
      }
      return loc;
    }
  ...
  public double getVolume() {
    ...
    for (RegionWrapper region : getRegions()) {
      count +=
          (region.maxX - (double) region.minX + 1) * (region.maxZ - (double) region.minZ + 1) * 256;
    }
    return count;
  }
  ...
  /**
   * Returns the top and bottom plot id.<br> - If the plot is not connected, it will return itself
   * for the top/bottom<br> - the returned ids will not necessarily correspond to claimed plots if
   * the connected plots do not form a rectangular shape
   *
      ...
      public void run(final CompoundTag value) {
        if (value == null) {
          ...
          TaskManager.runTaskAsync(new Runnable() {
            ...
            public void run() {
              String name =
                  Plot.this.id + "," + Plot.this.area + ',' + MainUtil.getName(Plot.this.owner);
              boolean result =
                  SchematicHandler.manager.save(value,
  ...
  /**
   * Export the plot as a BO3 object<br> - bedrock, floor and main block are ignored in their
   * respective sections - air is ignored - The center is considered to be on top of the plot in the
   * center
   *
  ...
  /**
   * Upload this plot as a BO3<br> - May not work on non default generator<br> - BO3 includes
   * flags/ignores plot main/floor block<br>
   *
   * @see BO3Handler
   */
    ...
    if (this.getSettings().setMerged(direction, value)) {
      if (value) {
        ...
        if (!other.equals(this.getBasePlot(false))) {
          Plot base =
              other.id.y < this.id.y || other.id.y == this.id.y && other.id.x < this.id.x ? other
                  : this.origin;
  ...
  public HashSet<RegionWrapper> getRegions() {
    ...
    for (Plot current : plots) {
      ...
      while (merge) {
        merge = false;
        ArrayList<PlotId> ids = MainUtil
            .getPlotSelectionIds(new PlotId(bot.x, bot.y - 1), new PlotId(top.x, bot.y - 1));
        boolean tmp = true;
        for (PlotId id : ids) {
        ...
        ids = MainUtil
            .getPlotSelectionIds(new PlotId(top.x + 1, bot.y), new PlotId(top.x + 1, top.y));
        tmp = true;
        for (PlotId id : ids) {
          Plot plot = this.area.getPlotAbs(id);
        ...
        ids = MainUtil
            .getPlotSelectionIds(new PlotId(bot.x, top.y + 1), new PlotId(top.x, top.y + 1));
        tmp = true;
        for (PlotId id : ids) {
          Plot plot = this.area.getPlotAbs(id);
        ...
        ids = MainUtil
            .getPlotSelectionIds(new PlotId(bot.x - 1, bot.y), new PlotId(bot.x - 1, top.y));
        tmp = true;
        for (PlotId id : ids) {
          Plot plot = this.area.getPlotAbs(id);
      ...
      for (int x = bot.x; x <= top.x; x++) {
        ...
        if (plot.getMerged(2)) {
          ...
          regions.add(
              new RegionWrapper(botabs.getX(), topabs.getX(), topabs.getZ() + 1, toploc.getZ()));
          if (plot.getMerged(5)) {
            regions.add(new RegionWrapper(topabs.getX() + 1, toploc.getX(), topabs.getZ() + 1,
                toploc.getZ()));
            // intersection
          }
        }
      ...
      for (int y = bot.y; y <= top.y; y++) {
        ...
        if (plot.getMerged(1)) {
          ...
          Location topabs = plot.getTopAbs();
          regions.add(
              new RegionWrapper(topabs.getX() + 1, toploc.getX(), botabs.getZ(), topabs.getZ()));
          if (plot.getMerged(5)) {
            regions.add(new RegionWrapper(topabs.getX() + 1, toploc.getX(), topabs.getZ() + 1,
                toploc.getZ()));
            // intersection
      ...
      regions
          .add(new RegionWrapper(gbotabs.getX(), gtopabs.getX(), gbotabs.getZ(), gtopabs.getZ()));
    }
    return regions;
  }
        ...
        if (regions.isEmpty()) {
          ...
          for (Plot current : plot.getConnectedPlots()) {
            ...
            Plot originPlot = originArea
                .getPlotAbs(new PlotId(current.id.x - offset.x, current.id.y - offset.y));
