...
public class SpongeUtil extends WorldUtil {
  ...
  public static Cause CAUSE = Cause.builder()
      .append(Sponge.getPluginManager().fromInstance(SpongeMain.THIS).get())
  ...
  public static Location getLocationFull(Entity player) {
    ...
    float[] pitchYaw = MathMan
        .getPitchAndYaw((float) rot.getX(), (float) rot.getY(), (float) rot.getZ());
    org.spongepowered.api.world.Location loc = player.getLocation();
    Vector3i pos = loc.getBlockPosition();
  ...
  public static Location getLocation(String world, Vector3d position) {
    return new Location(world, MathMan.roundInt(position.getX()), MathMan.roundInt(position.getY()),
        MathMan.roundInt(position.getZ()));
  }
  ...
  public String[] getSign(Location location) {
    ...
    Optional<TileEntity> block = world
        .getTileEntity(location.getX(), location.getY(), location.getZ());
    if (!block.isPresent()) {
      return null;
    }
