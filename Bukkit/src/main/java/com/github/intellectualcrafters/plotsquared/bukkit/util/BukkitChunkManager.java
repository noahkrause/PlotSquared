...
import com.github.intellectualcrafters.plotsquared.bukkit.object.entity.EntityWrapper;
...
import com.github.intellectualcrafters.plotsquared.plot.object.PlotLoc;
...
public class BukkitChunkManager extends ChunkManager {
  ...
  public boolean copyRegion(Location pos1, Location pos2, Location newPos,
    ...
    Location pos4 = new Location(newPos.getWorld(), newPos.getX() + relX, 256,
        newPos.getZ() + relZ);

    final RegionWrapper region = new RegionWrapper(pos1.getX(), pos2.getX(), pos1.getZ(),
        pos2.getZ());
    ...
    TaskManager.runTask(new Runnable() {
      ...
      public void run() {
        ...
        while (!chunks.isEmpty() && System.currentTimeMillis() - start < 5) {
          ...
          RegionWrapper currentPlotClear = new RegionWrapper(pos1.getX(), pos2.getX(), pos1.getZ(),
              pos2.getZ());
  ...
  public static class ContentMap {
    ...
    public void restoreBlocks(World world, int xOffset, int zOffset) {
      for (Entry<BlockLoc, ItemStack[]> blockLocEntry : this.chestContents.entrySet()) {
        try {
          Block block =
              world.getBlockAt(blockLocEntry.getKey().x + xOffset, blockLocEntry.getKey().y,
          ...
          if (state instanceof InventoryHolder) {
            ...
            PS.debug("&c[WARN] Plot clear failed to regenerate chest: " + (blockLocEntry.getKey().x
                + xOffset) + ',' + blockLocEntry
                .getKey().y + ',' + (blockLocEntry.getKey().z + zOffset));
          }
        } catch (IllegalArgumentException ignored) {
          PS.debug(
              "&c[WARN] Plot clear failed to regenerate chest (e): " + (blockLocEntry.getKey().x
                  + xOffset) + ',' + blockLocEntry
                  .getKey().y + ',' + (blockLocEntry.getKey().z + zOffset));
        }
      }
      for (Entry<BlockLoc, String[]> blockLocEntry : this.signContents.entrySet()) {
        try {
          ...
          if (state instanceof Sign) {
            ...
            PS.debug(
                "&c[WARN] Plot clear failed to regenerate sign: " + (blockLocEntry.getKey().x
                    + xOffset) + ',' + blockLocEntry
                    .getKey().y
                    + ',' + (
                    blockLocEntry.getKey().z + zOffset));
          ...
          PS.debug("&c[WARN] Plot clear failed to regenerate sign: " + (blockLocEntry.getKey().x
              + xOffset) + ',' + blockLocEntry.getKey().y
              + ',' + (
              blockLocEntry.getKey().z + zOffset));
        }
      }
      for (Entry<BlockLoc, ItemStack[]> blockLocEntry : this.dispenserContents.entrySet()) {
        try {
          ...
          if (state instanceof Dispenser) {
            ...
            PS.debug(
                "&c[WARN] Plot clear failed to regenerate dispenser: " + (blockLocEntry.getKey().x
                    + xOffset) + ',' + blockLocEntry
                    .getKey().y + ',' + (blockLocEntry.getKey().z + zOffset));
          }
        } catch (IllegalArgumentException ignored) {
          PS.debug(
              "&c[WARN] Plot clear failed to regenerate dispenser (e): " + (blockLocEntry.getKey().x
                  + xOffset) + ',' + blockLocEntry
                  .getKey().y + ',' + (blockLocEntry.getKey().z + zOffset));
        }
      }
      ...
      for (Entry<BlockLoc, ItemStack[]> blockLocEntry : this.beaconContents.entrySet()) {
        try {
          ...
          if (state instanceof Beacon) {
            ...
            PS.debug("&c[WARN] Plot clear failed to regenerate beacon: " + (blockLocEntry.getKey().x
                + xOffset) + ',' + blockLocEntry
                .getKey().y + ',' + (blockLocEntry.getKey().z + zOffset));
          }
        } catch (IllegalArgumentException ignored) {
          PS.debug(
              "&c[WARN] Plot clear failed to regenerate beacon (e): " + (blockLocEntry.getKey().x
                  + xOffset) + ',' + blockLocEntry
                  .getKey().y + ',' + (blockLocEntry.getKey().z + zOffset));
        }
      }
      for (Entry<BlockLoc, Material> blockLocMaterialEntry : this.jukeboxDisc.entrySet()) {
        try {
          Block block =
              world.getBlockAt(blockLocMaterialEntry.getKey().x + xOffset,
                  blockLocMaterialEntry.getKey().y, blockLocMaterialEntry
                      .getKey().z + zOffset);
          BlockState state = block.getState();
      ...
      for (Entry<BlockLoc, Object[]> blockLocEntry : this.skullData.entrySet()) {
        try {
          ...
          if (state instanceof Skull) {
            ...
            PS.debug("&c[WARN] Plot clear failed to restore skull: " + (blockLocEntry.getKey().x
                + xOffset) + ',' + blockLocEntry
                .getKey().y + ',' + (blockLocEntry.getKey().z + zOffset));
          }
        } catch (Exception e) {
          PS.debug(
              "&c[WARN] Plot clear failed to regenerate skull (e): " + (blockLocEntry.getKey().x
                  + xOffset) + ',' + blockLocEntry
                  .getKey().y + ',' + (blockLocEntry.getKey().z + zOffset));
          e.printStackTrace();
        }
      ...
      for (Entry<BlockLoc, ItemStack[]> blockLocEntry : this.hopperContents.entrySet()) {
        try {
          ...
          if (state instanceof Hopper) {
            ...
            PS.debug("&c[WARN] Plot clear failed to regenerate hopper: " + (blockLocEntry.getKey().x
                + xOffset) + ',' + blockLocEntry
                .getKey().y + ',' + (blockLocEntry.getKey().z + zOffset));
          }
        } catch (IllegalArgumentException ignored) {
          PS.debug(
              "&c[WARN] Plot clear failed to regenerate hopper (e): " + (blockLocEntry.getKey().x
                  + xOffset) + ',' + blockLocEntry
                  .getKey().y + ',' + (blockLocEntry.getKey().z + zOffset));
        }
      }
        ...
        try {
          ...
          if (state instanceof NoteBlock) {
            ...
            PS.debug("&c[WARN] Plot clear failed to regenerate note block: " + (
                blockLocNoteEntry.getKey().x + xOffset) + ','
                + blockLocNoteEntry
                .getKey().y + ',' + (
                blockLocNoteEntry.getKey().z + zOffset));
          }
      ...
      for (Entry<BlockLoc, Short> blockLocShortEntry : this.brewTime.entrySet()) {
        try {
          Block block = world.getBlockAt(
              blockLocShortEntry.getKey().x + xOffset, blockLocShortEntry.getKey().y,
          ...
          if (state instanceof BrewingStand) {
            ...
            PS.debug("&c[WARN] Plot clear failed to restore brewing stand cooking: " + (
                blockLocShortEntry.getKey().x + xOffset) + ','
                + blockLocShortEntry
                .getKey().y + ',' + (
                blockLocShortEntry.getKey().z + zOffset));
          }
      ...
      for (Entry<BlockLoc, EntityType> blockLocEntityTypeEntry : this.spawnerData.entrySet()) {
        try {
          Block block =
              world.getBlockAt(blockLocEntityTypeEntry.getKey().x + xOffset,
                  blockLocEntityTypeEntry.getKey().y, blockLocEntityTypeEntry
                      .getKey().z + zOffset);
          BlockState state = block.getState();
          ...
          PS.debug("&c[WARN] Plot clear failed to restore spawner type (e): " + (
              blockLocEntityTypeEntry.getKey().x + xOffset) + ','
              + blockLocEntityTypeEntry.getKey().y + ',' + (blockLocEntityTypeEntry.getKey().z
              + zOffset));
        }
      }
      for (Entry<BlockLoc, String> blockLocStringEntry : this.cmdData.entrySet()) {
        try {
          Block block = world.getBlockAt(
              blockLocStringEntry.getKey().x + xOffset, blockLocStringEntry.getKey().y,
        ...
        try {
          ...
          if (state instanceof BrewingStand) {
            ...
            PS.debug("&c[WARN] Plot clear failed to regenerate brewing stand: " + (
                blockLocEntry.getKey().x + xOffset) + ','
                + blockLocEntry
                .getKey().y + ',' + (
          ...
          if (state instanceof Furnace) {
            ...
            PS.debug(
                "&c[WARN] Plot clear failed to restore furnace cooking: " + (
                    blockLocEntry.getKey().x + xOffset) + ',' + blockLocEntry
                    .getKey().y + ',' + (blockLocEntry.getKey().z + zOffset));
          }
        } catch (Exception ignored) {
          PS.debug(
      ...
      for (Entry<BlockLoc, ItemStack[]> blockLocEntry : this.furnaceContents.entrySet()) {
        try {
          ...
          if (state instanceof Furnace) {
            ...
            PS.debug(
                "&c[WARN] Plot clear failed to regenerate furnace: " + (blockLocEntry.getKey().x
                    + xOffset) + ',' + blockLocEntry
                    .getKey().y + ',' + (blockLocEntry.getKey().z + zOffset));
          }
        } catch (IllegalArgumentException ignored) {
          PS.debug(
              "&c[WARN] Plot clear failed to regenerate furnace (e): " + (blockLocEntry.getKey().x
                  + xOffset) + ',' + blockLocEntry
                  .getKey().y + ',' + (blockLocEntry.getKey().z + zOffset));
        }
      }
      for (Entry<BlockLoc, DyeColor> blockLocByteEntry : this.bannerBase.entrySet()) {
        try {
          ...
          if (state instanceof Banner) {
            ...
            PS.debug(
                "&c[WARN] Plot clear failed to regenerate banner: " + (blockLocByteEntry.getKey().x
                    + xOffset) + ','
                    + blockLocByteEntry.getKey().y + ',' + (blockLocByteEntry.getKey().z
                    + zOffset));
          }
        } catch (Exception ignored) {
          PS.debug("&c[WARN] Plot clear failed to regenerate banner (e): " + (
              blockLocByteEntry.getKey().x + xOffset) + ','
        ...
        if (!id.equals(Material.AIR)) {
          try {
            ...
            if (block.getState() instanceof InventoryHolder) {
              ...
              this.skullData.put(bl,
                  new Object[]{skull.hasOwner(), skull.getOwner(), skull.getRotation(),
                      skull.getSkullType()});
