...
public class NbtFactory {
  ...
  private NbtFactory() {
    if (this.BASE_CLASS == null) {
      try {
        ...
        // Prepare NBT
        this.COMPOUND_CLASS = getMethod(0, Modifier.STATIC, offlinePlayer, "getData")
            .getReturnType();
        this.BASE_CLASS = this.COMPOUND_CLASS.getSuperclass();
        this.NBT_GET_TYPE = getMethod(0, Modifier.STATIC, this.BASE_CLASS, "getTypeId");
        this.NBT_CREATE_TAG = getMethod(Modifier.STATIC, 0, this.BASE_CLASS, "createTag",
            byte.class);
        ...
        this.SAVE_COMPOUND = getMethod(Modifier.STATIC, 0, this.STREAM_TOOLS, null, this.BASE_CLASS,
            DataOutput.class);
  ...
  /**
   * Set the NBT compound tag of a given item stack. <p>
   *
  ...
  /**
   * Construct a wrapper for an NBT tag stored (in memory) in an item stack. This is where auxiliary
   * data such as enchanting, name and lore is stored. It does not include items material, damage
   * value or count. <p> The item stack must be a wrapper for a CraftItemStack.
   *
  ...
  /**
   * Convert a given NBT element to a primitive wrapper or List/Map equivalent. <p> All changes to
   * any mutable objects will be reflected in the underlying NBT element(s).
   *
  ...
  private static class LoadMethodSkinUpdate extends LoadCompoundMethod {
    ...
    LoadMethodSkinUpdate(Class<?> streamClass, Class<?> readLimiterClass) {
      setMethod(
          getMethod(Modifier.STATIC, 0, streamClass, null, DataInput.class, readLimiterClass));
  ...
  /**
   * Represents a root NBT compound. <p> All changes to this map will be reflected in the underlying
   * NBT compound. Values may only be one of the following: <ul> <li>Primitive types</li> <li>{@link
   * String String}</li> <li>{@link NbtList}</li> <li>{@link NbtCompound}</li> </ul> <p> See also:
   * <ul> <li>{@link NbtFactory#createCompound()}</li> <li>{@link NbtFactory#fromCompound(Object)}</li>
   * </ul>
   *
   * @author Kristian
   */
  public final class NbtCompound extends ConvertedMap {
    ...
    /**
     * Set the value of an entry at a given location. <p> Every element of the path (except the end)
     * are assumed to be compounds, and will be created if they are missing.
     *
    ...
    /**
     * Retrieve the value of a given entry in the tree. <p> Every element of the path (except the
     * end) are assumed to be compounds. The retrieval operation will be cancelled if any of them
     * are missing.
     *
  ...
  /**
   * Represents a root NBT list. See also: <ul> <li>{@link NbtFactory#createList(Iterable)}}</li>
   * <li>{@link NbtFactory#fromList(Object)}</li> </ul>
   *
   * @author Kristian
   */
  ...
  /**
   * Represents a list that wraps another list and converts elements of its type and another exposed
   * type.
   *
   * @author Kristian
   */
